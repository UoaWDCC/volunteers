      ...
      <Link to={`/events/${imgIndex + 1}`}>
        <div className='image flex overflow-hidden aspect-[8/3]'>
          {data.map((event, index) => (
            <img src={event.imgB} key={index} className='picture object-cover w-[100%] h-[100%] shrink-0 grow-0 transition-translate 700ms ease-in-out duration-700' style={{ translate: `${-100 * imgIndex}%` }} />
          ))}
        </div>
      </Link>

      <img className='absolute z-20 right-0 bottom-[-0.07px] w-[100%]' src='..\public\assets\gallery\borders\GalleryBord.svg' />
