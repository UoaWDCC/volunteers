...
export default function EventDetails({ event, setEventDetails }: EventProps) {
  ...
  return (
    <div className="flex flex-col items-center fixed left-[15%] top-[12%] w-[85%] h-[90%] px-5 py-5 bg-[#F7F7FB] overflow-scroll scrollbar-none">
      {" "}
      {/* event-container */}
      <div
        className="self-start ml-6 cursor-pointer"
        onClick={() => setEventDetails(null)}
      >
        <IoArrowBackCircle size="40px" color="#3B87DD" />
      </div>
      ...
      <div className="flex flex-col w-full px-10 py-4">
        <div className="flex flex-row justify-between items-center w-full">
          ...
          <button className="h-10 text-body-heading rounded-full">
            Register for Event
          </button>
        </div>
        ...
        <div className="flex flex-row justify-start gap-3 w-full">
          {event.tag.map((tag, index) => (
            <div
              key={index}
              className="text-xs pt-[3px] px-3 h-6 text-grey border-grey border-solid border-2 rounded-full "
            >
              {tag}
            </div>
          ))}
      ...
      <div className="flex flex-row w-full justify-between">
        <div className="w-[65%]">
          ...
          <div className="flex flex-col w-full px-10 mt-3">
            <h3 className="text-heading3 font-semibold">
          ...
          <div className="flex flex-col w-full px-10 mt-3">
            ...
            <p className="text-body-heading mt-[-20px]">
        ...
        <div className="flex flex-col w-[35%] pr-10">
          <div className="w-full h-[35vh] object-cover">
            <iframe
              title="map"
              className="w-full h-full border-lightGrey border-solid border-2 rounded-lg"
              src={mapEmbed}
            />
          </div>
          ...
          <div className="flex flex-col w-full pl-2">
            ...
            <p className="text-body-heading mt-[-20px]">
              {startDate.toLocaleString("en-NZ", options)}
            </p>
          </div>
